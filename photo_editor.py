...
class Operations:
    ...
    def __str__(self):
        return "size: " + str(self.size) + ", filter: " \
                                           "" + str(self.color_filter) + ", " + "" \
                                                                                "b: " + str(self.brightness) + " " \
                                                                                                               "c: " + str(
            self.contrast) + " s: " + str(self.sharpness) + "," \
                                                            " " + "flip-left: " + str(self.flip_left) + " flip-top: " \
                                                                                                        "" + str(
            self.flip_top) + " rotation: " + str(self.rotation_angle)
...
class ActionTabs(QTabWidget):
    ...
    def __init__(self, parent):
        super(ActionTabs, self).__init__()
        self.parent = parent
...
class RotationTab(QWidget):
    ...
    def __init__(self, parent):
        super(RotationTab, self).__init__()
        self.parent = parent
...
class ModificationTab(QWidget):
    ...
    def __init__(self, parent):
        super(ModificationTab, self).__init__()
        self.parent = parent
    ...
    def on_width_change(self, e):
        logger.debug("type width " + self.width_box.text())
        global _img_original
    ...
    def on_height_change(self, e):
        logger.debug("type height " + self.height_box.text())
        global _img_original
        if self.ratio_check.isChecked():
...
class AdjustingTab(QWidget):
    ...
    def __init__(self, parent):
        super(AdjustingTab, self).__init__()
        self.parent = parent
    ...
    def on_brightness_slider_released(self):
        logger.debug("brightness selected value: " + self.brightness_slider.value())
        ...
        logger.debug("brightness factor: " + factor)
    ...
    def on_sharpness_slider_released(self):
        ...
        logger.debug("sharpness factor: " + str(factor))
    ...
    def on_contrast_slider_released(self):
        ...
        logger.debug("contrast factor: " + factor)
...
class FiltersTab(QWidget):
    ...
    def __init__(self, parent):
        super(FiltersTab, self).__init__()
        self.parent = parent
    ...
    def add_filter_thumb(self, name, title=""):
        logger.debug("create lbl thumb for: " + name)
        ...
        if name != "none":
            thumb_lbl.setToolTip("Apply <b>" + title + "</b> filter")
        else:
    ...
    def on_filter_select(self, filter_name, e):
        logger.debug("apply color filter: " + filter_name)
    ...
    def toggle_thumbs(self):
        for thumb in self.findChildren(QLabel):
            ...
            thumb.setStyleSheet("border:2px solid " + color + ";")
...
class MainLayout(QVBoxLayout):
    ...
    def on_save(self):
        ...
        new_img_path = QtGui.QFileDialog.getSaveFileName(self.parent, 'Save File', self.file_name,
                                                          "Images;;png( *.png);;jpg( *.jpg)")

        if new_img_path:
            print(new_img_path)
            logger.debug("save output image to " + new_img_path)
            img = _get_img_with_all_operations()
            img.save(str(new_img_path))
            global output_path
    ...
    def on_upload(self, img_path):
        ...
        if img_path:
            ...
            imageUri = QtCore.QUrl(QtCore.QString("file://{0}".format(img_path)))
    ...
    def update_img_size_lbl(self):
        ...
        global _img_original
        self.img_size_lbl.setText("<span style='font-size:11px'>" +
                                  "image size " + str(operations.size[0]) if operations.size else str(
            _img_original.width) + " X " +
                                                                                                  str(operations.size[
                                                                                                          1]) if operations.size else str(
...
class EasyPzUI(QDialog):
    """Main widget"""

    def __init__(self, filepath):
        super(EasyPzUI, self).__init__()

        self.filepath = filepath
        self.main_layout = MainLayout(self, filepath)
        self.setLayout(self.main_layout)

        self.setMinimumSize(600, 500)
        self.setMaximumSize(900, 900)
        self.setGeometry(600, 600, 600, 600)
        self.setWindowTitle('Insert Image and Edit')
        self.center()
        # self.show()
